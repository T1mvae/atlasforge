...
**Load order** (from `index.html` / `Map Editor.html`): `vendor/` libs → `js/i18n.js` → `core.js` → `storage.js` → `pwa.js` → `geo.js` → `regions.js` → `export.js` → `edit.js` → `terrain-algos.js` → `world.js` → `atlas.js` → `build/map.js` → `build/world.js` → `build/panels.js` → `build/chrome.js` → `build/app.js` (the last five are `js/*.jsx` precompiled). Edit the sources in `js/`, never `build/`.
