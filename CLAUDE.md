...
- `world.js` — `World`: the **custom world** basemap (`kind: "world"`). Painted rasters in memory: `World.height` (Int16 metres, `RW×RH` = 2000×1020, `RES` = 2 cells per data unit) and `World.cover` (Uint8, 0 = climate decides). Brushes (relief, cover, line tools ridge/valley/river) work on per-stroke base copies and push **raster diff undo entries** (`Actions.pushUndoEntry`). Rendering: an HTML `<canvas>` (Horn hillshade, biome colours, snow from temperature) placed *under* the SVG by `World.place`. `World.requestHydro` runs climate + hydrology in `js/terrain.worker.js`; `World.generate` runs smart provinces there and applies them to `project.customGeo`.
- `terrain-algos.js` — `TerrainAlgos`: pure typed-array algorithms shared by page and worker on the **data grid** (1000×510): climate, Priority-Flood, flow/accumulation, evaporation-balanced lakes, rivers + Strahler order, basin merging, **smart provinces** (blocks by landform × basin → geodesic Voronoi atoms → crest-aware merging: a province never contains mountain pieces separated by a crest), exact label polygons and per-arc topology smoothing. Test it in Node (`globalThis.self = globalThis; require(...)`).
- `atlas.js` — `Atlas`: `analyze` (province/landmass/water/range statistics on the data grid), `remapPolitics` (owners carried across re-cuts by pixel-overlap vote), `names`, `build` (the Markdown atlas for AI). `world.jsx` holds the palette, size rail, river card and atlas modal.
