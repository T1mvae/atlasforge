...
- **Deploy:** push to `main` → GitHub Pages serves the repo root. `index.html` is the Pages entry and is a byte-identical copy of `Map Editor.html`; keep them in sync (both `<script>` the same `js/` files). The sandboxed preview browser cannot load the github.io URL — verify deploys with `curl`/`gh` (HTTP 200 + grep the deployed `js/`).
